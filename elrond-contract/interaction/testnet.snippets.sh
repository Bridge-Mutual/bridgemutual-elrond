...
#mint_tokens() {
    ...
    erdpy --verbose contract call ${ADDRESS} --recall-nonce --pem=${ALICE} --gas-limit=5000000 --function="mint_tokens" --arguments ${NUMBER} --send --proxy=${PROXY} --chain=T
#}

#token_total_supply() {
    erdpy --verbose contract query ${ADDRESS} --function="token_total_supply" --proxy=${PROXY}
#}
